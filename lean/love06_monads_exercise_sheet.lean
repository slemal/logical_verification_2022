...
Hint: Use `simp [(>>=)]` to unfold the definition of the bind operator and
`simp [(<|>)]` to unfold the definition of the `orelse` operator. -/
...
* The lemma `faction.pure_apply` or `faction.bind_apply` might prove useful. -/
